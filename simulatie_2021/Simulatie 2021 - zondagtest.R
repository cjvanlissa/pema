...
devtools::install_github("cjvanlissa/pema")
...
packagenames <- c('devtools', 'metaforest', 'parallel', 'sn', 'brms', 'rstan', 'metafor', 'rstantools')


# Load packages
library(parallel) # For parallel computing
library(pema) #for brma function and simulate_smd
library(sn) #for rsn function which allows skewed distributions
library(metaforest) # for random effects metaforest
library(metafor) #for rma function
library(rstan) #to compile C++ in Rstudio
library(brms) #for package Rcpp that interfaces C++ code into R
library(rstantools)

# Check package versions (which packages require correct versions and which versions are they?)--------------------------------------------------

versions <- c(
  compareVersion(as.character(packageVersion("metaforest")), "0.1.3"),
  compareVersion(as.character(packageVersion("pema")), "0.1.0.9000"),
  compareVersion(as.character(packageVersion("metafor")), "3.0.2"))
if(!all(versions == 0)) stop("Using the incorrect version of one or more packages.")

# Load simulation functions from source -----------------------------------
##### -------------- this is not necessary anymore as all functions  are in the libraries
#source("simulate_smd.R")
source("model_accuracy.R") #this one i dont know
#source("mf_sim.R")
#source("rma_sim.R")
#source("crt_sim.R")
#source("rma_for_sim.R")
...
#---since all functions are loaded within packages, I assume we dont need this anymore
clusterExport(cl, "simulate_smd")
clusterExport(cl, "model_accuracy") #This one does seem important
...
# Conduct MF models -------------------------------------------------------
  ...
  #mf_sim(cl, simulated_data = simulated_data, file_stem = "mf_f", num.trees = 10, whichweights = "fixed")
  #mf_sim(cl, simulated_data = simulated_data, file_stem = "mf_u", num.trees = 10, whichweights = "unif")
  MetaForest(cl, num.trees = 150, data = simulated_data, file_stem = "mf_r")

# Conduct rma models ------------------------------------------------------

  #rma_sim(cl, simulated_data = simulated_data, file_stem = "rma")
  rma(cl, data = simulated_data, file_stem = "rma")

# Conduct crt (pema) models ------------------------------------------------------

  #crt_sim(cl, simulated_data = simulated_data, file_stem = "crt")
  brma(cl, data = simulated_data, method = 'lasso')
  brma(cl, data = simulated_data, method = 'hs')
