# Load packages
packagenames <- c('pema', 'metaforest', 'parallel', 'sn', 'rstan', 'bayestestR', 'foreach', 'glmnet')
...
if(!all(versions > -1)) stop("Using the incorrect version of one or more packages.")
...
summarydata$moderators <-  summarydata$moderators + 1
...
# Run a random sample of test iterations
summarydata <- summarydata[sample.int(nrow(summarydata), 4, replace = FALSE), ]
f <- list.files("./simulatie_2021/results", full.names = TRUE)
file.remove(f)
out_sel <- rep(NA, max(summarydata$moderators))
foreach(rownum = 1:nrow(summarydata), .packages = c("pema", "sn", "metaforest", "rstan", "bayestestR", "glmnet"), .final = function(x) NULL) %dopar% {
...
# Conduct rma models ------------------------------------------------------
  ...
  # Conduct twostep models ------------------------------------------------------

  models <- tryCatch({
    fit_las <- cv.glmnet(x = as.matrix(data$training[,-c(1:2)]),
                         y = data$training$yi,
                         alpha = 1,
                         nfolds = 5)
    mods <- as.matrix(data$training[,-c(1, 2, (which(coef(fit_las, s = "lambda.min")[-1,1] == 0) + 2)), drop = FALSE])
    rma(yi = data$training$yi,
        vi = data$training$vi,
        mods = mods)},
    error = function(e){ NULL })
  if(!is.null(models)){
    fits <-
      c(rownum,
        model_accuracy(
          fit = models,
          newdata = models$X[,-1, drop = FALSE],
          observed = data[[1]]$yi
        ),
        model_accuracy(
          fit = models,
          newdata = as.matrix(data$testing[, rownames(models$b)[-1]]),
          observed = data$testing$yi,
          ymean = mean(data$training$yi)
        ),
        tau2 = models$tau2,
        beta1 = tryCatch(models$b["X1",1], error = function(e){0})
      )
    out <- c(rownum, out_sel)
    out[2:(ncol(data$training)-1)] <- names(data$training)[-c(1:2)] %in% rownames(models$b)
  } else {
    fits <- c(
      rownum,
      rep(NA, 8))
  ...
  write.table(x = t(fits), file = sprintf("./simulatie_2021/results/lrm_%d.txt" , Sys.getpid()), sep = "\t", append = TRUE, row.names = FALSE, col.names = FALSE)
  write.table(x = t(out), file = sprintf("./simulatie_2021/results/lrm_sel_%d.txt" , Sys.getpid()), sep = "\t", append = TRUE, row.names = FALSE, col.names = FALSE)
  ...
  if(!is.null(models)){
    ...
    fits <-
      c(rownum,
        ...
        tau2 = mean(models$fit@sim$samples[[1]]$`sd_1[1]`^2),
        beta1 = mean(models$fit@sim$samples[[1]]$`betas[1]`),
        divergent = sum(rstan:::get_sampler_params(models$fit)[[1]][3001:6000, "divergent__", drop = TRUE]), # Drop warmups
