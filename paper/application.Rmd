...
```{r data}
tab_desc <- descriptives(bonapersona)[c("name", "type", "n", "unique", "mean", "sd", "v")]
DT::datatable(tab_desc) %>%
  formatRound(columns=c("mean", "sd", "v"), digits = 2)
```
...
For this application, we use a smaller selection of moderators than in Bonapersona et al. (2019; see Figure 3A for an overview).

```{r dummies}
datsel <- bonapersona %>% 
  select(yi, vi, author, mTimeLength, year, model, ageWeek, strainGrouped, bias, species, domain, sex) 
```
...
DT::datatable(sum$coefficients[, c("mean", "sd", "2.5%", "97.5%", "n_eff", "Rhat")]) %>%
  formatRound(columns=c("mean", "sd", "2.5%", "97.5%", "n_eff", "Rhat"), digits = 2)
```
...
parsel <- df1 %>%
  ...
  select(par) %>%
  filter(!par %in% c("Intercept", "tau2"))
