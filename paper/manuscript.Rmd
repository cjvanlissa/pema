...
Penalized regression analyses often require the scales of the predictors to be equalized ([@tibshirani1996; @lee2015note]). This is because $\lambda$, the regularization parameter, penalizes the magnitude of the coefficients that are dependent on the scales of the predictors. This can lead to the unfair penalization of larger coefficients that do not necessarily reflect importance ([@zou2006adaptive]). To clarify, a regression parameter $\beta$ can be interpreted as the amount outcome $Y$ increases as predictor $X$ increases with 1 unit. This means that as $X$ is reduced by a factor of 10, its regression coefficient is increased by a factor of 10. So suppose the desire to predict running speed by a person`s height. If height is measured in kilometers, than a one unit increase in height, which is a one kilometer increase, will likely lead to a drastic increase in running speed. In penalized regression techniques, this high value for $\beta$ would then dominate the cost function, leaving coefficients of other predictors unregularized.

To equalize predictor scales, an often-used technique is standardization ([@gelman2008scaling]). Standardization sets the mean of all predictors to 0 and their standard deviation to 1. It does so by transforming all observations in the predictor space $\boldsymbol{X}$ to Z-scores as follows:

$$
\mathbf{Z} = \frac{\mathbf{X}- \bar{\mathbf{X}}}{\mathbf{s_X}}
$$
where $\mathbf{X}$ is the matrix of predictors, $\bar{\mathbf{X}}$ is a vector of predictor means and $\mathbf{s_X}$ is a vector of standard deviations of the predictors. `brma()` standardizes the predictors by default, on which the model is estimated. The function returns the estimated parameters back in their original scales. This is achieved for the intercept by applying:

$$
b_0 = b_{0Z} - \mathbf{b}_Z\frac{\bar{\mathbf{X}}}{\mathbf{s}_X} \\
$$
where $b_0$ is the intercept, $b_{0Z}$ is the intercept for the normalized predictors and $\mathbf{b_Z}$ is the vector of regression coefficients for the normalized predictors. The regression coefficients are returned in their original scale by applying:

$$
\mathbf{b_X} = \frac{\mathbf{b_Z}}{\mathbf{s_X}}
$$

There is some debate regarding the standardization of binary/dummy predictors in penalized regression ([@lee2015note]). Some suggest to always standardize dummy variables ([@tibshirani1997lasso]). However, standardizing binary predictors increases the absolute maximum values the predictor can take on which in some cases can make its coefficient more sensitive to the penalty function ([@harrell2001regression]). 

In any case, the default standardization option in `brma()` can be overwritten entirely by setting `standardize = False`. `brma()` also allows the user to standardize only some of the predictors. This is done by constructing a vector of means and a vector of standard deviations. These vectors contain the means and standard deviations of the predictors in their original scales, except for the predictors that **should not** be standardized. For the latter, a mean of 0 and a standard deviation of 1 must be passed. The predictors that need to be standardized should then be standardized manually. Finally, the mean and standard deviation vectors are passed to the `standardize` argument as follows: `standardize = list(center = MeanVector, scale = SdVector)`. 
