...
In general, an estimator with low bias and high variance produces results that generalize worse to new data than an estimator with high bias and low variance.
...
\end{align}
The resulting parameter estimates perfectly describe linear relations in the present data set, but do not generalize well to new data.
...
its value can be estimated during model estimation.
...
$s = 1$, $\nu_1 = 1$, which are the same defaults as used in `brms` [@R-brms_a].
...
To overcome these limitations, the `pema` package includes pre-compiled stock models with opinionated default options. These models have been created using the `brms` package [@R-brms_a].
...
Default values for the LASSO prior were based on the defaults used in @R-brms_a,
