...
bibliography      : ["r-references.bib", "references.bib", "sara-references.bib"]
...
<!-- Eli? -->
<!-- Staclexchange: Lasso regression puts constraints on the size of the coefficients associated to each variable. However, this value will depend on the magnitude of each variable. It is therefore necessary to center and reduce, or standardize, the variables. -->
...
In Lasso regression it is often preferred to equalize the scales of the predictors (REF). This is because $\lambda$, the regularization parameter, constrains the absolute magnitude of the coefficients that are dependent on the scales of the predictors. This leads to Lasso unfairly penalizing large coefficients that do not necessarily reflect importance (REF).
Equalizing scales is usually done by setting the mean of all predictors to 0 and their variance to 1, a procedure known as standardization. This procedure transforms all elements in a predictor vector to Z-scores as follows:
$\boldsymbol{Z} = \frac{\boldsymbol{X} - \mu_\boldsymbol{X}}{\sigma_\boldsymbol{X}}$
where $\boldsymbol{X}$ is the vector of the predictor, $\mu_\boldsymbol{X}$ is its mean and $\sigma_\boldsymbol{X}$ is its standard deviation.
