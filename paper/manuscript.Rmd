...
Penalized regression analyses often require the scales of the predictors to be equalized ([@tibshirani1996; @lee2015note]). This is because $\lambda$, the regularization parameter, penalizes the magnitude of the coefficients that are dependent on the scales of the predictors. This leads to the unfair penalization of larger coefficients that do not necessarily reflect importance ([@zou2006adaptive]). For example, in the case of 2 or more equally important predictors, the coefficient of the predictor with the largest scale will be shrunk most.

To equalize predictor scales, an often-used technique is standardization ([@gelman2008scaling]). Standardization sets the mean of all predictors to 0 and their variance to 1. It does so by transforming all observations in the predictor space $\boldsymbol{X}$ to Z-scores as follows:
