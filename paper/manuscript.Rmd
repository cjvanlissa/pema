...
Lasso often requires the scales of the predictors to be equalized ([@tibshirani1996]). This is because $\lambda$, the regularization parameter, penalizes the absolute magnitude of the coefficients that are dependent on the scales of the predictors. This leads to the Lasso unfairly penalizing larger coefficients that do not necessarily reflect importance (REF). For example, in the case of 2 or more equally important predictors, the coefficient of the predictor with the largest scale will be shrunk most.

To equalize predictor scales, an often-used technique is standardization. Standardization sets the mean of all predictors to 0 and their variance to 1. It does so by transforming all cases in the predictor space $\boldsymbol{X}$ to Z-scores as follows:
$$
\mathbf{Z_{i}} = \frac{\mathbf{X}_i- \bar{\mathbf{X}}}{\mathbf{s_X}}
$$
where $\mathbf{X}$ is the matrix of predictors, $\bar{\mathbf{X}}$ is a vector of predictor means and $\mathbf{s_X}$ is a vector of standard deviations of the predictors. In BRMA, the predictors are standardized by default, on which the model is estimated. The function returns the estimated parameters in their original scales. This is achieved for the intercept by applying:
$$
b_0 = b_{0Z} - \mathbf{b}_Z\frac{\bar{\mathbf{X}}}{\mathbf{s}_X} \\
$$
...
In the case of binary/dummy predictors, 
