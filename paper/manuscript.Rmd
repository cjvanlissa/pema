...
# Applied example
In this application, we will work with the bonapersona data (@BonapersonaEtal2019) which is available in the `pema` package.In this study, over 400 rodent experiments where meta-analysed to investigate the effects of early life adversity on cognitive performance. The study includes over 30 potential moderators. Here, we select a small random subset of moderators to illustrate the regularized horseshoe prior. We focus specifically on how to choose the different hyperparameters of the regularized horseshoe prior.

<!-- Caspar: misschien is het handig om de data binnen het package aan te passen zodat de eerste stappen hieronder niet nodig zijn? Wil je daarnaast de code voor de toepassing ook nog in een apart bestand of wil je de snippets laten zien binnen het paper?
-->
...
# Use a small random subset of moderators
datsel <- bonapersona %>% 
  select(yi, vi, mTimeLength, year, model, ageWeek) 

# First recode categorical moderators
catmod <- c("model")
...
In any Bayesian analysis, it is a good idea to visualize the prior. One way to do so is through the shiny app that is implemented in the `pema` package. This app can be launched with the following command: `shiny_prior()` and provides the user the opportunity to plot draws from the prior distribution given specific values for the hyperparameters. It can be seen that increasing `scale_global` or `hs_scale_slab` will lead to a prior that is more spread out and will therefore pull coefficients less heavily towards zero. The degrees of freedom parameters influence the heaviness of the tails with higher values leading to thinner tails. Generally, it is not needed to adapt these hyperparameters and so we will only consider the default values.

A second way to gain insight into the prior distribution is by plotting the implied prior on the effective number of nonzero coefficients given specific hyperparameters. One advantage of the regularized horseshoe prior is its ability to include prior information on the number of nonzero coefficients. This can be done by setting the `relevant_pars` argument to the expected number of relevant moderators, which will determine the `scale_global` hyperparameter setting. If no prior information is given via the `relevant_pars` argument, the `scale_global` is fixed to 1. 
...
<!-- Caspar: zou het mogelijk zijn om dit nog als functie in te bouwen in het package? Het benodigde stan model staat nu bij de andere stan modellen.
-->

```{r exprior, eval = TRUE}
mod <- stan_model("./prior_meff.stan")
...
## Regularized meta-analysis with the horseshoe prior
Next, we run the model using the default hyperparameter settings.

<!-- Caspar: in de originele toepassing met 26 moderatoren kreeg ik een foutmelding als ik probeerde te standaardizeren via de brma functie zelf, maar nu doet ie het wel. Misschien iets om nog even naar te kijken hoe dat precies komt?
-->

```{r exmodel1, eval = TRUE}
fit.hs1 <- brma(yi ~ .,
                data = datdum,
                vi = "vi",
                method = "hs",
...
### Convergence
By setting `mute_stan = FALSE` we can see how far the sampling procedure is and we obtain all messages from Stan, the computational back-end of `brma()`. This is important to assess convergence of the sampler to the posterior distribution. Various numeric and visual diagnostics exist to assess convergence. If any serious convergence issues occur, Stan will provide warning messages. One warning message often seen with the horseshoe prior is that of divergent transitions after warmup. Divergent transitions can result in biased estimates, but if the number of divergences is small and the other convergence diagnostics do not indicate any issues, the posterior distribution is often good enough to safely interpret the results. In some cases, the number of divergences can be reduced by increasing the values for the degrees of freedom parameters. 

In this case, the default settings result in 111 divergent transitions. Increasing both `df_global` and `df_slab` to 5 results in slightly less divergences but does not have any substantial influence on the results.

```{r exmodel2, eval = TRUE}
fit.hs2 <- brma(yi ~ .,
                data = datdum,
                vi = "vi",
                method = "hs",
...
Traceplots can be created for each parameter to visualize the MCMC draws, for example using the `bayesplot` package. Ideally, these traceplots look like 'fat caterpillars' with the different MCMC chains mixing together. Note that there are many auxiliary parameters used for the regularized horseshoe prior. No information on these parameters is returned in the `brma` fitobject, however, once the fitobject is transformed into a Stan object for plotting, these parameters are included.

```{r extrace, eval = TRUE}
fit <- as.stan(fit.hs1)
posterior <- as.array(fit)
pars <- dimnames(posterior)$parameters
parsel <- c("Intercept", pars[grep("betas", pars)], "tau2")
mcmc_trace(posterior, pars = parsel)
```

Additionally, convergence can be checked manually by asking for the summary of the fitobject:

```{r exsumm, eval = TRUE}
summary(fit.hs1)
```

The column `Rhat` provides information on the split $\hat{R}$, a version of the potential scale reduction factor (PSRF; @GelmanRubin1992). `Rhat` values close to 1 indicate convergence. In addition, the column `n_eff` provides information on the number of effective MCMC samples, which should be high relative to the total number of samples. These diagnostics can be easily visualized using the `bayesplot` package.
...
All `Rhat` values are close to 0, indicating convergence. The ratios of effective number of MCMC samples to total number of MCMC samples are on the low side. An often used heuristic is to consider ratios smaller than 0.1 as problematic. Even though this is not the case here, it might still be a good idea to increase the number of MCMC iterations.

### Results
We can request the results using the `summary()` function, which provides the posterior mean estimate for the effect of each moderator. Since Bayesian penalization does not automatically shrink estimates exactly to zero, some additional criterion is needed to determine which moderators should be selected in the model. Currently, this is done using the 95% credible intervals, with a moderator being selected if zero is excluded in this interval. In the `summary` this is denoted by an asterisk for that moderator. In this model, there are no moderators for which the 95% CI excludes 0. The results can also be visualized.

```{r exres, eval = TRUE}
summary(fit.hs1)

# plot
df <- as.data.frame(summary(fit.hs1)[[1]])
df$par <- rownames(df)
plotdat <- filter(df, !par %in% c("Intercept", "tau2"))
ggplot(plotdat, aes(x = mean, y = par)) +
  geom_point() +
...
## Prior sensitivity
Finally, we run the model with different prior hyperparameters to assess the sensitivity of the results to the chosen values of the hyperparameters. Specifically, we focus on different settings for the expected number of relevant moderators `relevant_pars`. 

```{r expriorsens, eval = TRUE}
fit.hs3 <- brma(yi ~ .,
                data = datdum,
                vi = "vi",
                method = "hs",
...
df3 <- df.fun(fit.hs3, prior = "hs with p0 = 1")
df4 <- df.fun(fit.hs4, prior = "hs with p0 = 3")
df5 <- df.fun(fit.hs5, prior = "hs with p0 = 6")
