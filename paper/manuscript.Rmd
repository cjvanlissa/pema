...
abstract: |
  ...
  A key challenge is that the number of these potential moderators is often high
  relative to the number of studies.
  This introduces a risk of overfitting and finding spurious results.
  To overcome this challenge,
  [we introduce Bayesian Regularized Meta-Analysis (BRMA),
  which selects relevant moderators from a larger set of candidates
  by shrinking small regression coefficients towards zero
  with regularizing (LASSO or horseshoe) priors.]{#lassoabstract}
  ...
  moderators,
  but the overall proportion of Type I and Type II errors was
  equivalent to RMA.
...
bibliography      : ["r-references.bib", "references.bib",
                     "sara-references.bib", "eli-references.bib", "metaforest.bib"]
...
The present study introduces a novel method that overcomes this challenge by imposing Bayesian regularizing (LASSO and horseshoe) priors on the regression coefficients.
...
<span id = "theory">One way to perform variable selection is by relying on theory,
and selecting only moderators that should theoretically have an impact on effect size.
...
The implications of the ecological fallacy for interpreting the *results* of meta-regression have been discussed elsewhere [@bakerUnderstandingHeterogeneityMetaanalysis2009]:
For example, meta-regression may find a significant positive effect of average sample age on treatment efficacy,
even if age is uncorrelated with treatment effect within each study.
A similar problem arises when using individual level theory to select study level moderators:
...
For an example of study level theory,
consider the *decline effect*:
...
This prior work used the non-parametric *random forest* algorithm to perform variable selection.
...
In linear models, variable selection can be accomplished via a statistical method called  *regularization*.
...
the LASSO and horseshoe prior.</span>
...
This assumption is appropriate when meta-analyzing close replication studies [@higginsReevaluationRandomeffectsMetaanalysis2009].
...
This assumption is appropriate when studies are conceptually similar but differ in small random ways [@higginsReevaluationRandomeffectsMetaanalysis2009]. 
...
the most common of which is restricted maximum likelihood [REML, @pattersonRecoveryInterblockInformation1971].
...
There are some notable differences between the frequentist LASSO penalty and its Bayesian counterpart, the LASSO prior. First, the frequentist LASSO penalty has a tuning parameter $\lambda$, whose value is usually chosen via cross-validation. 
In the Bayesian approach, an analogous role is fulfilled by a hyperparameter of the shrinkage prior.
Instead of using cross-validation, the value of this hyperparameter can be estimated from the data.
...
One remaining limitation of the horseshoe prior is that it is difficult to specify a prior that ensures a sufficiently sparse solution.
The regularizing horseshoe was introduced to overcome this limitation;
it uses information about the expected number of parameters to determine an appropriate shrinkage prior  [@PiironenVehtari2017b].</span></span>
...
The regularizing horseshoe prior [@PiironenVehtari2017b] is given by:
$$
\begin{aligned}
...
This is accomplished by having the scale of the global shrinkage parameter $\lambda^2_0$ depend on the expected number of relevant moderators $p_0$.
The shrinkage parameter is then given by $\lambda^2_0 = \frac{p_0}{p-p_0}{\frac{\sigma}{\sqrt{n}}}$, where $p$ represents the total number of moderators, $\sigma$ is the residual standard deviation and $n$ equals the number of observations.</span>
...
<span id = "thisextension">The regularizing horseshoe differs from the standard horseshoe by the addition of a finite "slab", which is governed by a degrees of freedom parameter ($\nu_3$, set to 4) and a scale parameter ($s$, set to 1).
This slab ensures at least some regularization of large coefficients and as a consequence,
results in greater numerical stability of the model parameters.</span>
...
which merely guide the estimator towards possible values (e.g., by excluding negative values for the variance),
or restrict it to plausible values to aid model convergence.
...
BRMA performs standardization by default.
...
This so-called *regression specification* is useful when there is a meaningful reference category.
...
The intercept $b_0$ then represents the effect size of treatment as usual,
...
In this case, it makes more sense to estimate the average effect in both continents - in other words, to conduct a multi-group analysis.
...
In the context of standard linear regression, the regression specification and ANOVA specification are equivalent.
This is not the case in regularized regression, however, because shrinkage may affect the intercept differently from the dummy variables.
...
in the package `pema`, short for *pe*nalized *m*eta *a*nalysis [@R-pema].
...
Non-R-users can use BRMA via a graphical user interface by selecting the menu option "Penalized Meta-Analysis" in the free, open source statistical program JASP [@JASP2022], see Figure \@ref(fig:figjasp).
...
The regularizing horseshoe applies additional regularization to very large coefficients,
...
As explained before, standardization is an important step in Bayesian regularized meta-analysis.
...
and which prior (regularizing horseshoe versus LASSO) is to be preferred.
...
The number of cases in the testing data set was 100 for all replications.
The models under evaluation (BRMA, RMA) are estimated on the training data,
and used to predict cases in the testing data.
Predictive performance was operationalized as the model's explained variance in the testing data, $R^2_{test}$, which is calculated as follows:
$$
...
The RMA algorithm failed to converge in `r out$missing` cases.
These cases were all characterized by low number of cases ($k \leq 40$) and high effect sizes $\beta \geq .5$.
...
The variance of parameter estimates cannot be calculated on a case-by-case basis.
Instead, it is calculated across replications for each simulation condition.
Across simulation conditions, parameters estimated via HS had the lowest variance, $V_{HS} `r report(out[["beta_variance"]][["hs_beta1"]])`$, followed by LASSO, $B_{LASSO} `r report(out[["beta_variance"]][["lasso_beta1"]])`$,
...
the largest effect sizes were observed for the difference between HS and RMA in the effects of effect size, sample size, and model, all with $\eta^2 < 0.01$.
...
The variance of the residual heterogeneity was calculated across replications for each simulation condition.
...
All differences between algorithms were small, $\eta^2 \leq 0.002$.

# Applied example

In this application, we will work with the `bonapersona` data [@BonapersonaEtal2019].
...
<span id = "sensitivityanalysis">All simulations were conducted with default settings for the model's prior distributions.
